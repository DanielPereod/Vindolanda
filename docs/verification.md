...
- `npx playwright test --config e2e/nutrition.config.ts`: 5 passed. Covers creating a catalog food, the live profile target preview and save, recipe creation with per-serving macros, passing a planned day to the diary, and generating the shopping list plus creating a task. The suite runs an isolated Vite server on port 5176 and mocks every API response in memory, so it never touches account data.
